...
class Campuseventspage(webapp.RequestHandler):
   def get(self):
      ...
      if offset == '':
         ...
         offset = int(offset)
      content = ''
      query = Articlestable.all() #get articles from articletable ...
      query.filter('category','events') #filter query to obtain 'events' articles ...
      query.filter('campustag',campustag) #filter query to obtain 'events' from particular campus ...
